...
bool append_file(fs::FS &fs, String directory, String filename)
{
    //Serial.println(payloads);
    
    timerWrite(timer_w, 0); //reset timer (feed watchdog)
//Serial.println(temp_str2);
    if(directory != "/") {
        filename = directory + "/" + filename;
    }
    ...
    while (tmp) {
      //  Serial.println(message);
        ...
        if (!file) {
           ...
           /* if (file.print(payloads)) {
                log_message(DEBUG_MEM,  "SPIFFS:message appended");
                delay_ms(100);
                return true;
            }*/
            message = AppendSection.c_str();
            //Serial.println(message);
            }
            if (file.print(message)) {
                log_message(DEBUG_MEM,  "SPIFFS:message appended");
...
int read_int_value_from_eeprom(int eeprom_addr)
{
...
bool save_log_file(String path, String log_line) 
{
    ...
    AppendSection = log_line;
    if (!append_file(SPIFFS, path, log_name)) {
